...
#[test]
fn test_message_correct() {
    let msg = "Literally any string should be fine here";
    assert!(LogParser::parse(Rule::message, msg).is_ok());

    let msg_trailing = "   Even trailing spaces should theoretically be fine in this scenario  ";
...
//         datetime: NaiveDate::from_ymd_opt(2024, 11, 6).unwrap().and_hms_opt(22, 22, 0).unwrap(),
